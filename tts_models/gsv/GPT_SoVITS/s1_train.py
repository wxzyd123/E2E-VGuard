...
parser.add_argument('-n', "--exp_name", type=str, default="clean")
parser.add_argument("-b", "--batch_size", type=str, default="16")
parser.add_argument('-e', "--epoch", type=str, default="30")
parser.add_argument('-g', "--gpu_number", type=str, default="0")
parser.add_argument('-i', "--save_interval", type=str, default="5")
# args for dataset
...
def main(args):
    ...
    config["train"]["epochs"] = int(args.epoch)
    config["pretrained_s1"] = "checkpoints/GSV/base_models/gsv-v2final-pretrained/s1bert25hz-5kh-longer-epoch=12-step=369668.ckpt"
    config["train"]["save_every_n_epoch"] =int(args.save_interval)
    config["train"]["if_save_every_weights"] = False
    config["train"]["if_save_latest"] = False
    config["train"]["if_dpo"] = False
    config["train"]["half_weights_save_dir"] = "GPT_weights_v2"
    config["train"]["exp_name"] = exp_name
    config["train_semantic_path"] = "%s/6-name2semantic.tsv" % s1_dir
    config["train_phoneme_path"] = "%s/2-name2text.txt" % s1_dir
    config["output_dir"] = "%s/logs_s1"%s1_dir
    ...
    trainer: Trainer = Trainer(
        ...
        # val_check_interval=9999999999999999999999,
        limit_val_batches=0,
...
if __name__ == "__main__":
    ...
    #     help="path of config file",
    # )
    # parser.add_argument('-n', "--exp_name", type=str, default="UGC微调")
    # parser.add_argument("-b", "--batch_size", type=str, default="16")
    # parser.add_argument('-e', "--epoch", type=str, default="30")
    # parser.add_argument('-g', "--gpu_number", type=str, default="0")
    # # args for dataset
